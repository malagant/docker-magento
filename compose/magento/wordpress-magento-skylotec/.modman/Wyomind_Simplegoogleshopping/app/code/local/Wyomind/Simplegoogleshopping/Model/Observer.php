...
class Wyomind_Simplegoogleshopping_Model_Observer
{
                ...
                foreach ($cronExpr->days as $d) {
                    foreach ($cronExpr->hours as $h) {
                        ...
                        if (date('l', $cron['current']['gmtTime']) == $d) {
                            ...
                            $cron['tasks'][$i]['localTime'] = strtotime("last " . $d, $cron['current']['localTime'])
                                                                + ($time[0] * 60 * 60) + ($time[1] * 60);
                            $cron['tasks'][$i]['localDate'] = date('l Y-m-d H:i:s', $cron['tasks'][$i]['localTime']);
                        }

                        if ($cron['tasks'][$i]['localTime'] >= $cron['file']['localTime'] &&
                            $cron['tasks'][$i]['localTime'] <= $cron['current']['localTime'] &&
                            $done != true
                        ) {
    ...
    {
        if ($currency = (string) Mage::app()->getRequest()->getParam('currency')) {
