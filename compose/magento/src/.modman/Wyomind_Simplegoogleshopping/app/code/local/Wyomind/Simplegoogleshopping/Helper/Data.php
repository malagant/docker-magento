...
class Wyomind_Simplegoogleshopping_Helper_Data extends Mage_Core_Helper_Data
{
    ...
    public function checkHeartbeat()
    {
        ...
        if ($lastHeartbeat === false) {
            ...
            Mage::getSingleton('core/session')->addError(
                $this->__('No cron task found. '
                        . '<a href="https://www.wyomind.com/google-shopping-magento.html?section=faq#_scheduled-tasks-don-t-work" '
    ...
    '/^\//', '', $googleshopping->getSimplegoogleshoppingPath() . $googleshopping->getSimplegoogleshoppingFilename()
        ...
        $url = (Mage::app()->getStore($googleshopping->getStoreId())->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB) . $filePath);
        ...
        if ($io->fileExists(BP . DS . $filePath)) {
            ...
            if ($report == null) {
                $message = Mage::helper('simplegoogleshopping')->__('The data feed must be generated prior to any report.');

                return '<a href="' . $url . '?r=' . $urlTime . '" target="_blank">' . $url . '</a><br>[ ' . $message . ' ]';
    ...
    public function reportToHtml($data)
    {
        $notice = Mage::helper('simplegoogleshopping')->__(
            'This report does not replace the error report from Google and is by no means a guarantee that your data feed will be approved by the Google team.'
        );
        $html = null;
        ...
        foreach ($data['xml-error'] as $error) {
            ...
            if ($error['skus'] != '') {
                $html .= '<div class="examples">' . Mage::helper('simplegoogleshopping')->__('Examples:')
                        . ' <b>' . $error['skus'] . '</b></div>';
            }
        }
        ...
        if ($data == null) {
            return '<div id="blackbox-report">' . $html . '<br><br><b>'
                    . Mage::helper('simplegoogleshopping')->__('The data feed must be generated prior to any report.')
                    . '</b></div>';
        } elseif ($html !== null) {
            return '<div id="blackbox-report">' . $notice . $html . '</div>';
        } else {
            return '<div id="blackbox-report">' . $notice . '<br><br><b>'
    ...
    public function getRequirements()
    {
        $requirements = array(
            ...
            array(
                ...
                'depends' => array('google_product_category' => array('like' => 'Apparel & Accessories > Shoes'))
            ),
            array(
                'label' => 'size_system',
                'tag' => 'g_size_system',
