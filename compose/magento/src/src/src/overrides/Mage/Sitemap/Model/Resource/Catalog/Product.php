...
class Mage_Sitemap_Model_Resource_Catalog_Product extends Mage_Sitemap_Model_Resource_Catalog_Abstract
{
    ...
    public function getCollection($storeId)
    {
        ...
        if (Mage::helper('genticdesign')->isLineplanEnabled($storeId)) {
            $lineplan = Mage::getSingleton('genticdesign/catalog_lineplan');  
            $lineplan->setStoreId($storeId);
            if ($lineplan->isSummer()) {
                $this->_addFilter(
                    $storeId,
                    Conlabz_GenticDesign_Model_Catalog_Lineplan::ATTR_LINEPLAN_SUMMER,
                    1
                );
            }
            if ($lineplan->isWinter()) {
                $this->_addFilter(
                    $storeId, 
                    Conlabz_GenticDesign_Model_Catalog_Lineplan::ATTR_LINEPLAN_WINTER,
                    1
                );
            }
        }
